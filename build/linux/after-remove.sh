...
if [ -n "$target_user" ] && command -v systemctl >/dev/null 2>&1 && command -v runuser >/dev/null 2>&1; then
  ...
  if [ -n "$target_uid" ]; then
    runuser -u "$target_user" -- env XDG_RUNTIME_DIR="/run/user/$target_uid" systemctl --user disable --now nubem-drive-storage.service >/dev/null 2>&1 || true
  fi
fi

rm -f /usr/share/nemo/actions/nubem-cloud-folder.nemo_action
rm -f /usr/bin/nubem-drive-cloud-folder
rm -f /usr/bin/nubem-drive-storage
rm -f /usr/lib/systemd/user/nubem-drive-storage.service
